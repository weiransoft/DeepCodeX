...
test("B3. EAG_COMMAND_STRINGS 包含 7 个 EAG 命令字符串", () => {
  // 验证：EAG_COMMAND_STRINGS 常量包含 7 个 EAG 命令字符串（D-S3-6）
  ...
  // 验证总字段数（防止未来误新增）
  assert.equal(Object.keys(EAG_COMMAND_STRINGS).length, 7, "应有 7 个 EAG 命令字符串");
});
