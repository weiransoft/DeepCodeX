...
// 复用 fixtures 的 raw client → LLMClient 适配器（fork B1 compact 通路的测试缝合点）
import { createRawClientLLMClient } from "./fixtures/session-test-env";
  ...
  const manager = new SessionManager({
    ...
    getResolvedSettings: () => ({
      ...
      enabledSkills: {
        ...
        plan: false,
        // 合并说明：分叉在上游 4 个内置技能外新增了 17 个 bundled skill，
        // 本测试只验证 enabled-skill 的发现，新增的内置技能须显式禁用以隔离测试范围
...
test("createSession appends default system prompts in prefix-cache-friendly order", async () => {
  ...
  // 合并说明：分叉在系统提示词之后注入默认技能提示词（karpathy-guidelines 等 4 个
  // 隐式指南技能，V2.7 特性），因此运行时上下文顺延到 systemContents[2]
...
test("SessionManager resets active tokens to latest post-compaction response usage", async () => {
  ...
  const manager = createMockedClientSessionManager(workspace, responses, 131_072);
...
function createMockedClientSessionManager(
  ...
  return new SessionManager({
    ...
    renderMarkdown: (text) => text,
    onAssistantMessage: () => {},
    // 合并说明：fork B1 将 compact 非流式调用改经 createLLMClient（provider 抽象层）发起；
    // 此处把 raw client 适配为 LLMClient，保持上游语义（compact 与主对话共用同一响应队列）
...
function createMockedClientSessionManagerWithClient(projectRoot: string, client: unknown): SessionManager {
  return new SessionManager({
    ...
    onAssistantMessage: () => {},
    // 合并说明：fork B1 将 compact 非流式调用改经 createLLMClient（provider 抽象层）发起；
    // 此处把 raw client 适配为 LLMClient，保持上游语义（compact 与主对话共用同一 client）
    createLLMClient: () => createRawClientLLMClient(client),
  });
}
