/**
 * EAG 编排器统一装配模块（T1 装配下沉，2026-09-24）
 *
 * 设计依据：docs/dev/eag-web-sedimentation-fixes.md §2.1（T1 装配下沉·方案 A）
 *
 * 背景（原 CLI 版 ui/core/eag-orchestrator-assembly.ts 头注释契约，完整保留）：
 * - /eag-autonomous 三命令（B 域）与 /eag-graph（D 域）的引擎与命令 Handler
 *   在 core 层完整实现并有测试覆盖，但历史 CLI 生产装配路径（App.tsx → SessionManager）
 *   曾未注入 autonomousOrchestrator / graphLoopOrchestratorOptions，
 *   导致命令在真实 CLI 中全部 fail-closed 不可用（2026-07-31 FIX-1/FIX-2 修复）。
 * - T1 下沉后，本模块是 CLI / Web 共享的**唯一生产装配点（单一数据源）**，
 *   负责真实构造全部依赖并返回；调用方（CLI App.tsx / Web session-pool）注入 SessionManager。
 *
 * 设计契约（与 core session.ts 的既有契约对齐）：
 * - 失败安全：任一组件构造异常 → 返回 undefined 并记录日志，
 *   SessionManager 维持"未注入时命令不可用"的 fail-closed 降级，宿主主流程不崩溃。
 ...
 *
 * 下沉原则（方案 A 兼容性铁律）：
 * - 三个 build* 工厂签名、日志文案、fail-closed 语义与原 CLI 版逐字节对齐；
 * - CLI 专属依赖（装配日志文件写入器等）通过 log 形参注入，core 工厂不感知宿主日志载体；
 * - GoalDispatcher / 插件 / 编排器类型全部来自 core 包内部模块（相对路径导入，禁止自引用包名）。
 *
 * @module eag/assembly
 */
...
import type { AutonomousOrchestrator } from "./p5/index.js";
 ...
 *
 * 将 4 级日志统一桥接到 AssemblyLogCallback（CLI 侧最终汇入 eag-assembly.log，
 * Web 侧汇入服务端日志通道——由调用方决定 log 的具体载体）。
 *
 * @param log 装配日志回调
...
export function buildAutonomousOrchestrator(log: AssemblyLogCallback = () => {}): AutonomousOrchestrator | undefined {
  try {
    const orchestrator = createAutonomousOrchestrator({
      ...
      runStateStore: new P5RunStateStore((message, level) => log(`[P5RunStateStore] ${message}`, level)),
    ...
    // 失败安全：装配失败不阻断宿主启动，/eag-autonomous 维持 fail-closed 降级
...
/**
 * 生产级 LoopHandoffAdapter（原 CLI 版 CliLoopHandoffAdapter，T1 下沉 core 后更名）
 *
 * 为图遍历中的 loop 节点提供真实的 Handoff（executor）/ Verification（evaluator）回调。
 * 该实现只依赖 core 内 GoalDispatcher / GraphLogger / child_process，无 React / 终端 UI
 * 依赖，故可整体下沉供 CLI / Web 共享（评审结论：无需保留 CLI 本地）。
 *
 * 真实实现承诺（架构师红线）：
 ...
 */
export class ProductionLoopHandoffAdapter implements LoopHandoffAdapter {
  ...
  createLoopExecutor(
    ...
    return async (iteration, currentInput, _context, feedback) => {
      // fail-closed：未配置 plugin 的 loop 节点在生产路径无真实执行体
      // （自然语言任务需要 LLM 子会话，属于后续接线范围），诚实返回失败
      if (!pluginName) {
        this.logger.warn(`[ProductionLoopHandoffAdapter:${nodeId}] loop 节点未配置 plugin，无法真实执行`, {
          iteration,
        });
        return {
          success: false,
          error: `loop 节点 ${nodeId} 未配置 plugin 字段，生产路径无法执行（fail-closed）`,
        };
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : String(err);
        this.logger.error(`[ProductionLoopHandoffAdapter:${nodeId}] executor 派发异常：${reason}`, { iteration });
  ...
  private evaluateByTestCommand(
    ...
    return new Promise((resolve) => {
      exec(command, { cwd: this.projectRoot, timeout: TEST_COMMAND_TIMEOUT_MS }, (error, stdout, stderr) => {
        ...
        this.logger.warn(`[ProductionLoopHandoffAdapter:${nodeId}] ${reason}`, { iteration });
        resolve({
          passed: false,
          evaluatorId: "cli-test-command",
          reason,
 ...
 *
 * 装配依赖（全部为 core 生产组件）：
 * - nodeExecutor：NodeExecutorImpl（GoalDispatcher 真实注册 6 个生产插件 + ProductionLoopHandoffAdapter）
...
export function buildGraphLoopOrchestratorOptions(
  ...
  try {
    ...
    const loopHandoffAdapter = new ProductionLoopHandoffAdapter({
      goalDispatcher,
      projectRoot,
      logger: graphLogger,
    });
    ...
    // 失败安全：装配失败不阻断宿主启动，/eag-graph 维持 fail-closed 降级
    const reason = err instanceof Error ? err.message : String(err);
 ...
 *
 * 与 CLI App.tsx 中 eagDynamicSuggester 的 createDecisionLLMClient 同源：
...
export function buildDesignOrchestrator(
  ...
  try {
    ...
    // 失败安全：装配失败不阻断宿主启动，/eag-design 维持 fail-closed 降级
    const reason = err instanceof Error ? err.message : String(err);
