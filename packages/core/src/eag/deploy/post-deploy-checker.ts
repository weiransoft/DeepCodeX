...
import type { PostDeployChecker, PostDeployCheckContext, PostDeployCheckResult, HealthEndpoint } from "../devops/types";
...
export class PostDeployCheckerImpl implements PostDeployChecker {
  ...
  public async check(context: PostDeployCheckContext): Promise<PostDeployCheckResult> {
    ...
    const podsReady = await this.checkPodsReady(context.namespace);
    if (!podsReady) {
   ...
   * 2. JSON 解析失败 → 返回 false
   * 3. Pod 列表为空 → 返回 false（无 Pod 运行）
   * 4. 所有 Pod status.phase === "Running" → 返回 true
   * 5. 存在非 Running 状态的 Pod → 返回 false
   *
   * @param namespace 命名空间
   * @returns true=所有 Pod 就绪；false=Pod 未就绪或 kubectl 不可用
   */
  private async checkPodsReady(namespace: string): Promise<boolean> {
   ...
   * - 该端点返回 Prometheus 格式的指标数据
   * - 批次 13 仅校验端点可达性，批次 14 扩展：解析指标数据校验具体指标值
   *
   * @param namespace 命名空间（批次 13 未使用，批次 14 扩展时用于校验 namespace 级别指标）
   * @returns true=指标端点可达；false=指标端点不可达或 kubectl 不可用
   */
  private async checkMetricsReporting(namespace: string): Promise<boolean> {
    // 批次 13：namespace 参数未使用，仅校验全局 /metrics 端点
    // 批次 14 扩展：通过 namespace 参数校验 namespace 级别指标
    void namespace;
