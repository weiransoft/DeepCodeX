...
`deepseek-v4-pro`、`deepseek-v4-flash`等非多模态模型仍会使用`UnderstandImage`识图工具，图片会随多模态请求发送至你配置的模型服务，不会上传至任何外部插件服务器（隐私加固，2026-09-17）。Deep Code会自动判断模型能力，也可通过`multimodal`配置项手动覆盖。
