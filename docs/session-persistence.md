...
Deep Code 会把每个项目的会话记录保存在本机用户目录中。会话历史用于 `/resume`、`/fork`、`/continue` 和 `/undo`，不依赖当前终端进程是否仍在运行。
...
`/fork` 会复制当前活动会话并切换到新会话。命令行中的 `--fork <session-id>` 会复制指定会话；单独使用 `-f` 或 `--fork` 时，会复制当前项目最近更新的会话。`--fork` 不能与 `--resume` 或 `--last` 同时使用。
